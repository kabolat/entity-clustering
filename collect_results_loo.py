...
def main():
    ...
    for num_topics, num_lda_clusters, num_lower_dims, distance_measure_name, linkage in product(NUMBER_OF_LDA_TOPICS, NUMBER_OF_LDA_CLUSTERS, NUMBER_OF_LOWER_DIMS, DISTANCE_MEASURES, LINKAGE):
        ...
        for num_clusters in NUMBER_OF_CLUSTERS:
            ...
            for u in range(num_users): 
                cluster_members = np.where(labels==labels[u])[0]
                cluster_members = cluster_members[cluster_members != u]
                quantile_levels_entity[:,u] = fnq.nanquantile(X_train[cluster_members], quantiles, axis=0)
            
            _, quantile_losses = calculate_quantile_loss(targets=X_train, quantile_predictions=quantile_levels_entity, quantiles=quantiles, nonzero=True)
    ...
    results_path = os.path.join(results_folder, "results_loo.csv")
